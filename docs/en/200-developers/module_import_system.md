...
**try_find_module_in(module_name, root)** ([src/vm/file_import.rs](../../../src/vm/file_import.rs)) — for the **last** dotted segment and single-name imports:
...
**try_find_path_segment(segment, full_name, root)** — for **intermediate** dotted segments:

1. `<root>/<segment>/__lib__.dc` → package directory (unchanged behavior).
2. `<root>/<segment>/` exists → **namespace folder** (directory without `__lib__.dc`; `current_base` = that folder).
3. `<root>/<segment>.dc` exists but no directory → error (segment is a file; cannot import `foo.bar`).
4. Else → `Module '…' not found (package segment '…')`.

Search order: **base_path** (set from the running script or from the importing module’s directory), then **DPM package paths** (get_dpm_package_paths()). The first dotted segment is also resolved from **project_root** when set on the VM.
