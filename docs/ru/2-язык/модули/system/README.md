...
| `system.process` | Shell (`exec`) и **управление CPU/GPU** ([process-compute.md](process-compute.md)) |
...
| `gpu_count()` | Число GPU (Metal при `--features metal`) |
| `gpu_info()` | Массив `{name, backend, detail}` |
...
- Подробнее о встроенных модулях и импорте: [Модули и импорты](../../../0-синтаксис/11-модули-и-импорты.md).
