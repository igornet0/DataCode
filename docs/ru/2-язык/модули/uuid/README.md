...
u = uuid.v4()
bytes = uuid.to_bytes(u)
restored = uuid.from_bytes(bytes)
...
**Пример:**
```datacode
u = uuid.v3(uuid.URL, "https://example.com/page")
print(str(u))
...
**Пример:**
```datacode
u = uuid.v5(uuid.DNS, "example.com")
print(str(u))
...
- **Русский:** [examples/ru/12-uuid/](../../../../../examples/ru/12-uuid/)
...
```bash
datacode --bin datacode examples/ru/12-uuid/01-базовое_использование.dc
```
