...
- Создание моделей данных: [`examples/ru/08-создание модели данных/`](../../../../examples/ru/08-создание%20модели%20данных/)
...
### Инкрементальное добавление строк (`add_row`)

Вместо цикла `push` + пересборки через `table()` можно наращивать таблицу методом **`add_row`**:
...
Длина массива в `add_row` должна совпадать с числом колонок. Для объединения уже готовых таблиц используйте **`merge_tables([t1, t2])`**.
...
- `table.add_row(row)` - Добавить строку in-place (метод, не глобальная функция)
- `table_row_number(table, column_name?, start_from?)` - Добавить колонку с нумерацией строк (метод: `table.row_number(...)`)
- `table_distinct(table, columns?)` - Удалить дубли строк (метод: `table.distinct(...)`)
- `table_sample(table, n)` - Случайная выборка строк

**📚 Подробнее:** См. [Функции таблиц](../функции/таблицы.md)

---

**См. также:**
- [Типы данных](./справочник-типов.md) - подробнее о типе Table
- [Функции таблиц](../функции/таблицы.md) — полный список встроенных функций для работы с таблицами
- [Примеры создания моделей данных](../../../../examples/ru/08-создание%20модели%20данных/) - практические примеры
