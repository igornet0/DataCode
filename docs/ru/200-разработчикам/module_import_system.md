...
**try_find_module_in(module_name, root)** ([src/vm/file_import.rs](../../../src/vm/file_import.rs)) — для **последнего** сегмента dotted-импорта и одиночных импортов:
...
**try_find_path_segment(segment, full_name, root)** — для **промежуточных** сегментов dotted-импорта:

1. `<root>/<segment>/__lib__.dc` → директория пакета (как раньше).
2. `<root>/<segment>/` существует → **namespace-папка** (директория без `__lib__.dc`; `current_base` = эта папка).
3. `<root>/<segment>.dc` существует, но директории нет → ошибка (сегмент — файл, нельзя `foo.bar`).
4. Иначе → `Module '…' not found (package segment '…')`.

Порядок поиска: **base_path** (из запускаемого скрипта или директории импортирующего модуля), затем **DPM package paths** (get_dpm_package_paths()). Первый сегмент dotted-импорта также ищется от **project_root** (если задан в VM).
