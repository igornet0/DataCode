...
| Поле | Назначение |
|------|------------|
| `stack` | Хранилище операндов: `Vec<TaggedValue>` (capacity растёт, физически сжимается на safe points). |
| `stack_sp` | Логический верх стека (watermark). Слоты `stack[stack_sp..]` — «мёртвая» зона до `truncate`. |
...
| `stack_start` | Watermark при входе в фрейм: операнды фрейма лежат в `stack[stack_start .. stack_sp)`. Ниже могут оставаться временные значения вызывающего (`fib(a) + fib(b)`). |
...
## Стек: watermark и safe points ([src/vm/core/stack.rs](../../../src/vm/core/stack.rs))

**Логический `Pop`** (`OpCode::Pop`) только уменьшает `stack_sp`; `Vec::pop` на каждой инструкции не вызывается. Снятые слоты недоступны через `peek`/`pop`, но физически могут оставаться в `stack` до safe point.

| Операция | Поведение |
|----------|-----------|
| `push` / `push_id` | Запись в `stack[stack_sp]`, затем `stack_sp += 1` (при необходимости `resize` вектора). |
| `pop` / `peek` | Индексация относительно `stack_sp` и `frame.stack_start` (underflow, если `stack_sp <= stack_start`). |
| `OpCode::Pop` | `discard_top` — только `stack_sp -= 1`. |
| **Return** | Взять возврат; `truncate` до `callee.stack_start` (сохраняет temps вызывающего ниже этой метки); положить возврат. |
| **Native call** | Аргументы снимаются через `pop_direct`; результат — `push_id`. Сохраняются ранее вычисленные операнды для составного `Call` (без усечения до `stack_start+1`). |
| **Exception unwind** | `truncate_to(handler.stack_height)` ([exceptions.rs](../../../src/vm/exceptions.rs)). |
| **Начало run** | `reset_execution_stack()` — `stack.clear()`, `stack_sp = 0`. |

**Освобождение heap-объектов:** логический `Pop` не вызывает `recycle_heap_pair`. Значения в `locals` и в `ValueStore` живут по обычным правилам. Опционально, с фичей **`deferred_stack_gc`**, на safe points сканируется мёртвая зона `stack[old_sp..new_sp)` и перерабатываются только `recyclable` heap-pair id, не попавшие в live set (стек, `CallFrame.slots`, scratch-исключения) — см. [stack_sweep.rs](../../../src/vm/memory/stack_sweep.rs).

Прямой доступ к `stack.len()` / `stack.pop()` в рантайме VM запрещён по соглашению: используйте API из `vm::stack` (`logical_len`, `available_in_frame`, `pop_direct`).
...
1. **run(chunk, argv_patch)**  
   Устанавливается RunContext, в VM сливаются `global_names` из chunk, при необходимости патчится LoadGlobal для argv. Вызывается `reset_execution_stack()`. Создаётся начальный `CallFrame` (`stack_start = 0`) и пушится. При включённой фиче profile вызывается `profile::set()`.
...
2. **Цикл**  
   ...
   - **execute_instruction(op, line, ...)**  
     Match по OpCode; изменение stack/slots/globals/store; при Call — снятие аргументов, `stack_start = stack_sp`, push фрейма; при Return — compact стека до `callee.stack_start` и push возврата (или `VMStatus::Return` для верхнего фрейма).
