#!/usr/bin/env python3
"""
Run a DCP package on the DataCode WebSocket server.

Requires:
  pip install websockets datacode-dcp
  (install DCP-python from repo: pip install -e path/to/DCP-python)
...
try:
    ...
    print(
        "Install datacode-dcp: pip install -e DCP-python",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
async def run_dcp(uri: str = WS_URI) -> None:
    ...
    print(f"Built DCP package ({len(package)} bytes, magic {package[:4]!r})")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Could not connect to {uri}", file=sys.stderr)
        print("Start server: datacode --websocket --host 0.0.0.0 --port 8899", file=sys.stderr)
        return 1
