#!/usr/bin/env python3
"""
Build and optionally send a DCP package with a post-model SQL section.

Requires datacode_dcp (pip install -e DCP-python) and websockets for --send.

Start server with build_model:
...
try:
    ...
    print("Install datacode_dcp: pip install -e DCP-python", file=sys.stderr)
    raise SystemExit(1)
...
def main() -> int:
    ...
    parser.add_argument("--bad-sql", action="store_true", help="Use invalid SQL")
    parser.add_argument("--send", metavar="URI", help="Send to WebSocket server")
    args = parser.parse_args()

    package = build_package(bad_sql=args.bad_sql)
    print(f"Built DCP ({len(package)} bytes)")
