...
pub fn compile_super_method_call(
    ...
    if let Expr::SuperMethodCall { method, args, line } = expr {
        *ctx.current_line = *line;
        ...
        let superclass_name = match &ctx.current_superclass {
            ...
            None => {
                return Err(LangError::ParseError {
                    message: "super can only be used in a class that extends another class"
                        .to_string(),
                    line: *line,
                    file: None,
                });
        ...
        ctx.chunk
            .global_names
            .insert(method_global_index, parent_method_name);
        ctx.chunk
            .write_with_line(OpCode::LoadGlobal(method_global_index), *line);
        ctx.chunk
            .write_with_line(OpCode::Call(args.len() + 1), *line); // +1 for this
