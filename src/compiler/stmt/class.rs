...
fn has_column_field(
    ...
    fn field_has_column_default(f: &ClassField) -> bool {
        f.default_value.as_ref().map_or(
            false,
            |e| matches!(e, Expr::Call { name, .. } if name == "Column"),
        )
    }
...
fn emit_instance_extends_table(
    ...
    ctx.chunk
        .write_with_line(OpCode::Constant(extends_const), line);
    let key_const = ctx
        .chunk
        .add_constant(Value::String("__extends_table".to_string()));
...
fn emit_instance_private_metadata(
    ...
    ctx.chunk
        .write_with_line(OpCode::Constant(private_fields_const), line);
    let key_const = ctx
        .chunk
        .add_constant(Value::String("__private_fields".to_string()));
    ctx.chunk.write_with_line(OpCode::Constant(key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
    ctx.chunk.write_with_line(OpCode::SetArrayElement, line);
    ...
    let defining_key_const = ctx
        .chunk
        .add_constant(Value::String("__private_field_defining_class".to_string()));
    ctx.chunk
        .write_with_line(OpCode::Constant(defining_key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
    ctx.chunk.write_with_line(OpCode::SetArrayElement, line);
    ctx.chunk
    ...
    let class_name_const = ctx
        .chunk
        .add_constant(Value::String(class_name.to_string()));
    ctx.chunk
        .write_with_line(OpCode::Constant(class_name_const), line);
    let class_key_const = ctx
        .chunk
        .add_constant(Value::String("__class_name".to_string()));
    ctx.chunk
        .write_with_line(OpCode::Constant(class_key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
    ctx.chunk.write_with_line(OpCode::SetArrayElement, line);
    ctx.chunk
...
fn extract_field_default_value(default_value: &Option<Expr>) -> Option<Value> {
    ...
    for arg in args {
        if let Arg::Named { name: n, value } = arg {
            if n == "default" {
                if let Some(v) = crate::compiler::constant_fold::evaluate_constant_expr(value)
                    .ok()
                    .flatten()
                {
                    return Some(v);
...
fn emit_instance_protected_metadata(
    ...
    ctx.chunk
        .write_with_line(OpCode::Constant(protected_fields_const), line);
    let key_const = ctx
        .chunk
        .add_constant(Value::String("__protected_fields".to_string()));
    ctx.chunk.write_with_line(OpCode::Constant(key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
    ctx.chunk.write_with_line(OpCode::SetArrayElement, line);
...
fn emit_instance_private_methods_metadata(
    ...
    ctx.chunk
        .write_with_line(OpCode::Constant(private_methods_const), line);
    let key_const = ctx
        .chunk
        .add_constant(Value::String("__private_methods".to_string()));
    ctx.chunk.write_with_line(OpCode::Constant(key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
    ctx.chunk.write_with_line(OpCode::SetArrayElement, line);
    ...
    let defining_key_const = ctx
        .chunk
        .add_constant(Value::String("__private_method_defining_class".to_string()));
    ctx.chunk
        .write_with_line(OpCode::Constant(defining_key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
...
fn emit_instance_protected_methods_metadata(
    ...
    ctx.chunk
        .write_with_line(OpCode::Constant(protected_methods_const), line);
    let key_const = ctx
        .chunk
        .add_constant(Value::String("__protected_methods".to_string()));
    ctx.chunk.write_with_line(OpCode::Constant(key_const), line);
    ctx.chunk
        .write_with_line(OpCode::LoadLocal(this_slot), line);
    ctx.chunk.write_with_line(OpCode::SetArrayElement, line);
    ...
    } = stmt
    {
        *ctx.current_line = *line;
        ...
        let is_settings_chain_early = superclass
            .as_ref()
            .map(|s| is_settings_descendant(s, &ctx.class_superclass))
            .unwrap_or(false);
        if is_settings_chain_early {
            let ep = extract_env_prefix_from_class_vars(public_variables, private_variables);
            ctx.class_settings_env_prefix
                .insert(name.clone(), ep.unwrap_or_else(String::new));
        }
        ...
        let private_field_names: Vec<String> =
            private_fields.iter().map(|f| f.name.clone()).collect();
        let protected_field_names: Vec<String> =
            protected_fields.iter().map(|f| f.name.clone()).collect();
        ...
        let public_field_names: Vec<String> =
            public_fields.iter().map(|f| f.name.clone()).collect();
        let class_private_var_names: Vec<String> =
            private_variables.iter().map(|v| v.name.clone()).collect();
        let class_protected_var_names: Vec<String> =
            protected_variables.iter().map(|v| v.name.clone()).collect();
        ...
        class_metadata.insert(
            "__class_private_vars".to_string(),
            Value::Array(std::rc::Rc::new(std::cell::RefCell::new(
                class_private_var_names
                    .iter()
                    .map(|n| Value::String(n.clone()))
                    .collect(),
            ))),
        ...
        class_metadata.insert(
            "__class_protected_vars".to_string(),
            Value::Array(std::rc::Rc::new(std::cell::RefCell::new(
                class_protected_var_names
                    .iter()
                    .map(|n| Value::String(n.clone()))
                    .collect(),
            ))),
        ...
        if extends_table {
            let col_names: Vec<Value> = private_fields
                ...
                .collect();
            class_metadata.insert(
                "__col_names".to_string(),
                Value::Array(Rc::new(RefCell::new(col_names))),
            );
        }
        ...
        let constructors_to_compile: Vec<&crate::parser::ast::Constructor> =
        ...
        if superclass.is_some() && constructors.is_empty() {
            ...
            if extends_table && !all_instance_fields.is_empty() {
                ...
                constructor_function.param_types =
                ...
                for param in &params {
                    let default = match param.default_value.as_ref().and_then(|e| {
                        crate::compiler::constant_fold::evaluate_constant_expr(e)
                            .ok()
                            .flatten()
                    }) {
                        Some(v) => Some(v),
                        None => {
                            let default = if param.name == "id"
                                && type_parts_any(param.type_annotation.as_ref(), |s| {
                                    s.to_lowercase().contains("int")
                                }) {
                                Value::Null
                            } else if type_parts_any(param.type_annotation.as_ref(), |s| {
                                s.contains("bool")
                            }) {
                                Value::Bool(false)
                            } else if type_parts_any(param.type_annotation.as_ref(), |s| {
                                ...
                                s.contains("str") || s.contains("string")
                            }) {
                ...
                ctx.scope
                    .globals
                    .insert(constructor_name.clone(), global_index);
                ctx.chunk
                    .global_names
                ...
                ctx.chunk
                    .write_with_line(OpCode::LoadGlobal(table_global_index), *line);
                ...
                let extends_key_const = ctx
                    .chunk
                    .add_constant(Value::String("__extends_table".to_string()));
                ctx.chunk
                    .write_with_line(OpCode::Constant(extends_true_const), *line);
                ctx.chunk
                    .write_with_line(OpCode::Constant(extends_key_const), *line);
                ctx.chunk
                    .write_with_line(OpCode::LoadLocal(this_slot), *line);
                ctx.chunk.write_with_line(OpCode::SetArrayElement, *line);
                ctx.chunk
                ...
                for (i, field) in all_instance_fields.iter().enumerate() {
                    ...
                    let field_name_const =
                        ctx.chunk.add_constant(Value::String(field.name.clone()));
                    ctx.chunk
                ...
                ctx.chunk
                    .global_names
                    .insert(global_index, constructor_name.clone());
                let constructor_constant_index =
                    ctx.chunk.add_constant(Value::Function(function_index));
                ctx.chunk
                    .write_with_line(OpCode::Constant(constructor_constant_index), *line);
                ctx.chunk
                    .write_with_line(OpCode::StoreGlobal(global_index), *line);

                ctx.class_constructor
                    .insert(name.clone(), (constructor_name.clone(), function_index));
            } else {
                ...
                let super_callable = {
                    ...
                    if let Some(&idx) = ctx.scope.globals.get(&constructor_name) {
                        ...
                        let has_constructor = (1..=10).any(|n| {
                            ctx.scope
                                .globals
                                .contains_key(&format!("{}::new_{}", superclass_name, n))
                        });
                        if has_constructor {
                            ctx.class_superclass
                                .insert(name.clone(), superclass_name.clone());
                            None
                        } else {
                            let idx = *ctx.scope.globals.get(superclass_name).ok_or_else(|| {
                                LangError::ParseError {
                                    message: format!(
                                        "Superclass '{}' not found in scope",
                                        superclass_name
                                    ),
                                    line: *line,
                                    file: None,
                ...
                if let Some((super_callable_name, supercall_global_index)) = super_callable {
                    let is_settings_chain =
                        is_settings_descendant(superclass_name, &ctx.class_superclass);
                    if is_settings_chain {
                        ctx.class_superclass
                            .insert(name.clone(), superclass_name.clone());
                        let ep =
                            extract_env_prefix_from_class_vars(public_variables, private_variables);
                        ctx.class_settings_env_prefix
                            .insert(name.clone(), ep.clone().unwrap_or_else(String::new));
                        ...
                        ctx.class_required_keys_value
                            .insert(name.clone(), required_keys_value);
                    }
                    ...
                    let mut constructor_function =
                        Function::new(constructor_name.clone(), param_count);
                    ...
                    ctx.current_function = Some(function_index);
                    ctx.scope.begin_scope();
                    for p in &["path", "required_keys", "model_config", "parent_field_name"]
                        [..param_count]
                    {
                    ...
                    if superclass_name == "Settings" {
                        ...
                        ctx.labels
                            .emit_jump(ctx.chunk, *line, true, use_arg_label)?; // JumpIfFalse: when model_config != Null, jump to use LoadLocal(2)
                        ...
                        ctx.chunk.write_with_line(
                            OpCode::LoadGlobal(MODEL_CONFIG_CLASS_LOAD_INDEX),
                            *line,
                        );
                        let model_config_name_const = ctx
                            .chunk
                            .add_constant(Value::String("model_config".to_string()));
                        ctx.chunk
                            .write_with_line(OpCode::Constant(model_config_name_const), *line);
                        ctx.chunk.write_with_line(OpCode::GetArrayElement, *line);
                        ...
                        ctx.chunk
                            .write_with_line(OpCode::Constant(null_const_sc), *line);
                        ctx.chunk.write_with_line(OpCode::Equal, *line);
                        let use_arg_label_sc = ctx.labels.create_label();
                        ctx.labels
                            .emit_jump(ctx.chunk, *line, true, use_arg_label_sc)?;
                        ctx.chunk.global_names.insert(
                            MODEL_CONFIG_CLASS_LOAD_INDEX,
                            CONSTRUCTING_CLASS_GLOBAL_NAME.to_string(),
                        );
                        ...
                        let model_config_name_const_sc = ctx
                            .chunk
                            .add_constant(Value::String("model_config".to_string()));
                        ctx.chunk
                            .write_with_line(OpCode::Constant(model_config_name_const_sc), *line);
                        ctx.chunk.write_with_line(OpCode::GetArrayElement, *line);
                        let after_label_sc = ctx.labels.create_label();
                        ctx.labels
                            .emit_jump(ctx.chunk, *line, false, after_label_sc)?;
                        ctx.labels
                    ...
                    if superclass_name != "Settings" {
                        ...
                        let name_key_const =
                            ctx.chunk.add_constant(Value::String("name".to_string()));
                        ctx.chunk
                            .write_with_line(OpCode::Constant(class_name_const), *line);
                        ctx.chunk
                            .write_with_line(OpCode::Constant(name_key_const), *line);
                        ctx.chunk
                            .write_with_line(OpCode::LoadLocal(this_slot), *line);
                        ctx.chunk.write_with_line(OpCode::SetArrayElement, *line);
                        ctx.chunk
                        ...
                        let super_const = ctx
                            .chunk
                            .add_constant(Value::String(superclass_name.clone()));
                        ctx.chunk
                            .write_with_line(OpCode::Constant(super_const), *line);
                        let super_key_const = ctx
                            .chunk
                            .add_constant(Value::String("__superclass".to_string()));
                        ctx.chunk
                            .write_with_line(OpCode::Constant(super_key_const), *line);
                        ctx.chunk
                            .write_with_line(OpCode::LoadLocal(this_slot), *line);
                        ctx.chunk.write_with_line(OpCode::SetArrayElement, *line);
                        ctx.chunk
                    ...
                    file: None,
                                ...
                                ctx.chunk
                                    .global_names
                                    .insert(factory_global_index, factory_name.clone());
                                let skip_label = ctx.labels.create_label();
                                let field_name_const =
                                    ctx.chunk.add_constant(Value::String(f.name.clone()));
                                    ...
                                    (factory_required_keys, factory_ctor_global)
                                {
                                    ...
                                    ctx.chunk.write_with_line(
                                        OpCode::LoadGlobal(factory_global_index),
                                        *line,
                                    );
                                    ctx.chunk.write_with_line(OpCode::Call(1), *line);
                                }
                            ...
                            .chain(public_fields.iter())
                        {
                            if let Some(ref _default_expr) = field.default_value {
                                if let Some(constant_value) =
                                    extract_field_default_value(&field.default_value)
                                {
                    ...
                    if is_settings_chain && !private_fields.is_empty() {
                        let private_field_names: Vec<String> =
                            private_fields.iter().map(|f| f.name.clone()).collect();
                        let defining_class_map: HashMap<String, String> = private_fields
                            .iter()
                            .map(|f| (f.name.clone(), name.clone()))
                            .collect();
                        emit_instance_private_metadata(
                            ctx,
                            *line,
                            this_slot,
                            name,
                    ...
                    ctx.scope
                        .globals
                        .insert(constructor_name_0.clone(), global_index_0);
                    ctx.chunk
                        .global_names
                        .insert(global_index_0, constructor_name_0.clone());
                    ...
                    if is_settings_chain {
                        ...
                        let model_config_name_const_0 = ctx
                            .chunk
                            .add_constant(Value::String("model_config".to_string()));
                        ctx.chunk.global_names.insert(
                            MODEL_CONFIG_CLASS_LOAD_INDEX,
                        ...
                        let empty_str_const_0 =
                            ctx.chunk.add_constant(Value::String(String::new()));
                        ctx.chunk
                            .write_with_line(OpCode::Constant(empty_str_const_0), *line);
                        ctx.chunk
                            .write_with_line(OpCode::Constant(required_keys_const_0), *line);
                        ctx.chunk.write_with_line(
                            OpCode::LoadGlobal(MODEL_CONFIG_CLASS_LOAD_INDEX),
                            *line,
                        );
                        ctx.chunk
                            .write_with_line(OpCode::Constant(model_config_name_const_0), *line);
                        ctx.chunk.write_with_line(OpCode::GetArrayElement, *line);
                        let null_const_0 = ctx.chunk.add_constant(Value::Null);
                        ctx.chunk
                            .write_with_line(OpCode::Constant(null_const_0), *line);
                        ctx.chunk
                            .global_names
                            .insert(global_index, constructor_name.clone());
                        ctx.chunk
                        ...
                        let parent_new_0_idx =
                            if let Some(&idx) = ctx.scope.globals.get(&parent_new_0_name) {
                                idx
                            } else {
                                ...
                                ctx.chunk
                                    .global_names
                                    .insert(idx, parent_new_0_name.clone());
                                idx
                            };
                        ctx.chunk
                            .global_names
                            .insert(parent_new_0_idx, parent_new_0_name.clone());
                        ctx.chunk
                            .write_with_line(OpCode::LoadGlobal(parent_new_0_idx), *line);
                        ctx.chunk.write_with_line(OpCode::Call(0), *line);
                    }
                    ctx.chunk
                        .write_with_line(OpCode::StoreLocal(this_slot_0), *line);
                    let class_name_const_0 = ctx.chunk.add_constant(Value::String(name.clone()));
                    let class_key_const_0 = ctx
                        .chunk
                        .add_constant(Value::String("__class_name".to_string()));
                    ctx.chunk
                        .write_with_line(OpCode::Constant(class_name_const_0), *line);
                    ctx.chunk
                        .write_with_line(OpCode::Constant(class_key_const_0), *line);
                    ctx.chunk
                    ...
                    ctx.chunk
                        .global_names
                        .insert(global_index_0, constructor_name_0.clone());
                    let constructor_constant_index_0 =
                        ctx.chunk.add_constant(Value::Function(function_index_0));
                    ctx.chunk
                        .write_with_line(OpCode::Constant(constructor_constant_index_0), *line);
                    ctx.chunk
                        .write_with_line(OpCode::StoreGlobal(global_index_0), *line);
                    ctx.class_constructor
                        .insert(name.clone(), (constructor_name_0.clone(), function_index_0));
                ...
                .chain(public_variables.iter())
            {
                ...
                ctx.chunk
                    .write_with_line(OpCode::Constant(var_name_const), *line);
                ctx.chunk.write_with_line(OpCode::GetArrayElement, *line);
                ctx.chunk
                    .write_with_line(OpCode::Constant(var_name_const), *line);
                ctx.chunk
        ...
        for constructor in constructors_to_compile {
            ...
            let mut constructor_function =
                Function::new(constructor_name.clone(), constructor.params.len());
            constructor_function.param_names =
                constructor.params.iter().map(|p| p.name.clone()).collect();
            constructor_function.param_types = constructor
                .params
                .iter()
            ...
            if let Some(ref super_name) = superclass {
                if let Some(ref delegate_args) = constructor.delegate_args {
                    ...
                    let (super_callable_name, supercall_global_index) = {
                        ...
                        if let Some(&idx) = ctx.scope.globals.get(&parent_constructor) {
                            ...
                            let idx = *ctx.scope.globals.get(super_name).ok_or_else(|| {
                                LangError::ParseError {
                                    message: format!(
                                        "Superclass '{}' not found in scope",
                                        super_name
                    ...
                    ctx.chunk
                        .global_names
                        .insert(supercall_global_index, super_callable_name);
                    ...
                    for f in &parent_private {
                        defining_class.insert(f.clone(), super_name.clone());
                    }
                    for f in &private_field_names {
                        defining_class.insert(f.clone(), name.clone());
                    }
                    emit_instance_private_metadata(
                        ctx,
                        *line,
                    ...
                    let parent_private_methods = ctx
                        .class_private_methods
                        .get(super_name)
                        .cloned()
                        .unwrap_or_default();
                    let merged_private_methods: Vec<String> = parent_private_methods
                        .iter()
                        .chain(private_method_names.iter())
                        .cloned()
                        .collect();
                    let mut method_defining_class: HashMap<String, String> = HashMap::new();
                    for m in &parent_private_methods {
                        method_defining_class.insert(m.clone(), super_name.clone());
                    }
                    for m in &private_method_names {
                        method_defining_class.insert(m.clone(), name.clone());
                    }
                    emit_instance_private_methods_metadata(
                        ctx,
                        *line,
                        this_slot,
                        &merged_private_methods,
                        &method_defining_class,
                    );
                    let parent_protected_methods = ctx
                        .class_protected_methods
                        .get(super_name)
                        .cloned()
                        .unwrap_or_default();
                    let merged_protected_methods: Vec<String> = parent_protected_methods
                        .iter()
                        .chain(protected_method_names.iter())
                        .cloned()
                        .collect();
                    emit_instance_protected_methods_metadata(
                        ctx,
                        *line,
                        this_slot,
                        &merged_protected_methods,
                    );
                    emit_instance_class_reference(ctx, *line, this_slot, class_global_index);
                } else {
                    ...
                    let super_args: Vec<Arg> = if !constructor.body.is_empty() {
                        ...
                        return Err(LangError::ParseError {
                            message: format!(
                                "Class '{}' extends '{}' but does not call super(...)",
                                name, super_name
                            ),
                            line: constructor.line,
                            file: None,
                ...
                ctx.chunk
                    .write_with_line(OpCode::StoreLocal(this_slot), *line);
                let defining_class: HashMap<String, String> = private_field_names
                    .iter()
                ...
                let method_defining_class: HashMap<String, String> = private_method_names
                    .iter()
            ...
            for (i, opcode) in compiled_chunk.code.iter().enumerate() {
                ...
                let marker = if is_set_array {
                    " <-- SetArrayElement"
                } else if is_get_array {
                    " <-- GetArrayElement"
                } else {
                    ""
                };
                debug_println!(
                    "[DEBUG compile_class]   IP {} (строка {}): {:?}{}",
                    i,
                    line_num,
                    opcode,
                    marker
            ...
            let set_array_count = compiled_chunk
                .code
                .iter()
                .filter(|op| matches!(op, OpCode::SetArrayElement))
                .count();
            ...
            ctx.chunk
                .write_with_line(OpCode::Constant(constructor_constant_index), *line);
            ctx.chunk
                .write_with_line(OpCode::StoreGlobal(constructor_global_index), *line);
        }
        ...
        ctx.chunk
            ...
            .insert(class_global_index, name.clone());
        ctx.chunk
            .write_with_line(OpCode::StoreGlobal(class_global_index), *line);
        ...
        if let Some(ref val) = nested_specs_for_class_value {
            ...
            ctx.chunk
                .write_with_line(OpCode::Constant(nested_const), *line);
            let key_const = ctx
                .chunk
                .add_constant(Value::String("__nested_specs".to_string()));
            ctx.chunk
                .write_with_line(OpCode::Constant(key_const), *line);
            ctx.chunk
                .write_with_line(OpCode::LoadGlobal(class_global_index), *line);
            ctx.chunk.write_with_line(OpCode::SetArrayElement, *line);
        }
        ...
        for (global_name, &ctor_global_index) in ctx.scope.globals.iter() {
            if let Some(suffix) = global_name.strip_prefix(&ctor_prefix) {
                if suffix.chars().all(|c| c.is_ascii_digit()) {
                    let key_const = ctx
                        .chunk
                        .add_constant(Value::String(format!("new_{}", suffix)));
                    ctx.chunk
                        .global_names
                        .insert(ctor_global_index, global_name.clone());
                    ctx.chunk
                        .write_with_line(OpCode::LoadGlobal(ctor_global_index), *line);
                    ctx.chunk
                        .write_with_line(OpCode::Constant(key_const), *line);
                    ctx.chunk
                        .write_with_line(OpCode::LoadGlobal(class_global_index), *line);
        ...
        ctx.class_private_fields
            .insert(name.clone(), private_field_names.clone());
        ctx.class_protected_fields
            .insert(name.clone(), protected_field_names.clone());
        ctx.class_private_methods
            .insert(name.clone(), private_method_names.clone());
        ctx.class_protected_methods
            .insert(name.clone(), protected_method_names.clone());
