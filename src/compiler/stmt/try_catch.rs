    ...
    } = stmt
    {
        *ctx.current_line = *line;
        ...
        if let Some(ref finally_label) = finally_label {
            ctx.labels
                .emit_jump(ctx.chunk, *ctx.current_line, false, *finally_label)?;
        } else {
            ctx.labels
                .emit_jump(ctx.chunk, *ctx.current_line, false, after_try_label)?;
        }
        ...
        for catch_block in catch_blocks {
            ...
            let error_type_index = catch_block
                .error_type
                .as_ref()
                .map(|error_type_name| ctx.get_error_type_index(error_type_name));
            ...
            ctx.chunk
                .write_with_line(OpCode::Catch(error_type_index), catch_block.line);
            ...
            // Генерируем EndCatch
            ctx.chunk
                .write_with_line(OpCode::EndCatch, catch_block.line);
            ...
            if let Some(ref finally_label) = finally_label {
                ...
                ctx.labels
                    .emit_jump(ctx.chunk, *ctx.current_line, false, *after_catch_label)?;
            }
        }
        ...
        if let Some(else_block) = else_block {
            ...
            if finally_label.is_none() {
                if let Some(ref after_catch_label) = after_catch_label {
                    ctx.labels
                        .mark_label(*after_catch_label, ctx.chunk.code.len());
                }
            }
        }
        ...
        ctx.chunk
            .write_with_line(OpCode::PopExceptionHandler, *line);
