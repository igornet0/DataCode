...
pub fn compile_unpack_pattern(
    ...
    let error_msg = format!(
        "Unpack pattern requires at least {} elements, but got array with length less than {}",
    ...
    let mut var_index = 0;
    compile_unpack_pattern_recursive(
        pattern,
        var_locals,
        &mut var_index,
        temp_local,
...
fn compile_unpack_pattern_recursive(
    ...
    let variadic_pos = pattern.iter().position(|p| {
        matches!(
            p,
            UnpackPattern::Variadic(_) | UnpackPattern::VariadicWildcard
        )
