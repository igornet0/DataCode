...
use crate::plot::{system, GuiCommand, RenderContent, WindowState};
    ...
    F: FnOnce() -> Result<(), String> + Send + 'static,
{
    ...
    let runtime_handle = thread::spawn(move || {
        ...
        let _ = system::get_plot_system()
            .proxy()
            .send_event(GuiCommand::RuntimeFinished);
        result
    });
    ...
    let mut window_waiters: HashMap<winit::window::WindowId, Arc<(Mutex<bool>, Condvar)>> =
        HashMap::new();
        ...
        .run(move |event, elwt| {
            ...
            match event {
                Event::UserEvent(cmd) => match cmd {
                    GuiCommand::CreateWindow {
                        ...
                        let mut window_builder =
                            WindowBuilder::new().with_title(&title).with_inner_size(
                        ...
                        match window_builder.build(elwt) {
                            Ok(window) => {
                                ...
                                let plot_window =
                                    PlotWindow::new(window, width, height, title.clone());
                            ...
                            Err(e) => {
                                let _ =
                                    response.send(Err(format!("Failed to build window: {:?}", e)));
                            }
                        }
                    }
                ...
                Event::WindowEvent {
                    ...
                    let should_exit = window_events::handle_window_close(
                        &mut windows,
                        window_id,
                        &mut window_waiters,
                    );
