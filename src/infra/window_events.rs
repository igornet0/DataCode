...
use crate::plot::command::{ChartData, ChartType};
use crate::plot::renderer::Renderer;
use crate::plot::{RenderContent, WindowState};
    ...
    if let Some(state) = windows.get_mut(&window_id) {
        ...
        if let RenderContent::Chart(chart_data) = &state.content {
            if let ChartType::Line = chart_data.chart_type {
                ...
                let (x_min_plot, x_max_plot, y_min_plot, y_max_plot) =
                    calculate_chart_bounds(chart_data);
        ...
        if let RenderContent::Chart(chart_data) = &state.content {
            if let ChartType::Line = chart_data.chart_type {
                if let Some(cursor_pos) = state.cursor_pos {
                    ...
                    let scale_factor = state.window.scale_factor();
                    let (left_margin, right_margin, top_margin, bottom_margin) =
                        Renderer::calculate_adaptive_margins(
                            scale_factor,
                            buffer_width,
                            buffer_height,
                        );
        ...
        if let Some(ref mut renderer) = state.renderer {
            match &state.content {
                RenderContent::Image(image, view_state) => {
                    ...
                    let _ = renderer.draw_image_with_transform(&*img, &state.window, view_state);
                }
                ...
                RenderContent::ImageGrid {
                    images,
                    rows,
                    cols,
                    titles,
                } => {
                    let _ = renderer.draw_image_grid(images, *rows, *cols, Some(titles));
                }
                RenderContent::Figure(figure_data) => {
                    ...
                    let _ =
                        renderer.draw_figure_from_data(figure_data.clone(), rows, cols, &titles);
                }
                RenderContent::None => {}
            }
        }
