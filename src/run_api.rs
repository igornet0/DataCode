//! Run / compile entrypoints, registry preload, and module export remapping helpers.
...
/// Preloaded parse-time registries (operator descriptors + native_call metadata) for one token stream.
pub struct PreloadContext {
    pub operator_registry: std::sync::Arc<vm::operator_registry::OperatorRegistry>,
    pub native_call_registry: std::sync::Arc<vm::native_call_registry::NativeCallParamRegistry>,
}

impl PreloadContext {
    pub fn new_for_tokens(
        tokens: &[lexer::Token],
        base_path: Option<&std::path::Path>,
    ) -> Result<Self, LangError> {
        Ok(Self {
            operator_registry: preload_operator_registry_for_parse(tokens, base_path)?,
            native_call_registry: preload_native_call_registry_for_parse(tokens, base_path)?,
        })
    }
}

/// Unified entry for common run combinations (see [`run_with_options`]).
#[derive(Default)]
...
/// Stable, documented entry: dispatches to `run` / `run_with_existing_vm` / `run_with_base_path` / `run_with_vm_and_path`.
pub fn run_with_options(opts: RunOptions<'_>) -> Result<Value, LangError> {
    let RunOptions {
        source,
        base_path,
        vm,
    ...
    match (base_path, vm) {
        (None, None) => run(source),
        (None, Some(vm)) => run_with_existing_vm(source, Some(vm)),
...
pub fn run_with_existing_vm(
    ...
    let preload = PreloadContext::new_for_tokens(&tokens, None)?;
    let mut parser =
        Parser::new_with_source_name_and_registry(tokens, None, preload.operator_registry.clone());
    ...
    let mut compiler =
        Compiler::new_with_source_and_native_registry(None, Some(preload.native_call_registry));
...
fn run_with_vm_internal_with_args(
    ...
    let tokens = lexer.tokenize()?;
    let preload = PreloadContext::new_for_tokens(&tokens, base_for_lib.as_deref())?;
    let mut parser = Parser::new_with_source_name_and_registry(
        tokens,
        source_name_str.as_deref(),
        preload.operator_registry.clone(),
    );
    let mut ast = parser.parse()?;
    crate::compiler::array_map_onehot_fusion::inject_ml_onehots_import(&mut ast);
    ...
    let mut compiler = Compiler::new_with_source_and_native_registry(
        source_name_str.as_deref(),
        Some(preload.native_call_registry),
    );
    let chunk = compiler.compile(&ast)?;
    let functions = compiler.get_functions();
