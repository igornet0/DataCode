...
use crate::common::{
    error::{ErrorType, LangError, StackTraceEntry},
    value::Value,
...
impl ExceptionHandler {
    ...
    pub fn runtime_error(frames: &[CallFrame], message: String, line: usize) -> LangError {
        let file = frames
            .last()
            .and_then(|f| f.function.chunk.source_name.as_deref());
        LangError::runtime_error_with_trace_and_file(
            message,
            line,
            file,
            Self::build_stack_trace(frames),
        )
    }

    pub fn runtime_error_with_type(
        frames: &[CallFrame],
        message: String,
        line: usize,
    ...
    pub fn runtime_error_with_source(
        ...
        LangError::runtime_error_with_source_and_trace(
            message,
            source,
            Self::build_stack_trace(frames),
        )
