    ...
    match &index_value {
        Value::String(key) => {
            ...
            if let (Some(Value::String(ref class_name)), Some(Value::Array(private_fields_rc))) =
                (map.get("__class_name"), map.get("__private_fields"))
            {
                ...
                if is_private_field {
                    let defining_class = map
                        .get("__private_field_defining_class")
                        .and_then(|v| {
            ...
            ) = (map.get("__class_name"), map.get("__protected_fields"))
            {
            ...
            if let (Some(Value::String(ref class_name)), Some(Value::Array(private_methods_rc))) =
                (map.get("__class_name"), map.get("__private_methods"))
            {
                ...
                if is_private_method {
                    let defining_class = map
                        .get("__private_method_defining_class")
                        .and_then(|v| {
                            if let Value::Object(rc) = v {
                                rc.borrow().get(key).and_then(|v| {
                                    if let Value::String(s) = v {
            ...
            ) = (map.get("__class_name"), map.get("__protected_methods"))
            {
                ...
                let from_class = class_name_opt.as_ref().and_then(|class_name| {
                    let from_globals = global_index_by_name(global_names, class_name)
                        ...
                        .map(|idx| {
                            ...
                            load_value(id, value_store, heavy_store)
                        });
...
pub(crate) fn set_object(
    ...
    let (is_class_protected_denied, class_protected_msg) = {
        let map = obj_rc.borrow();
        let is_protected_var = map.contains_key("__class_name")
            && key != "model_config"
            && map
                .get("__class_protected_vars")
                .and_then(|v| {
                    if let Value::Array(rc) = v {
                        Some(rc.borrow().iter().any(|v| {
                            if let Value::String(s) = v {
        ...
        if !is_protected_var {
            ...
            let obj_class = map.get("__class_name").and_then(|v| {
                if let Value::String(s) = v {
                    Some(s.clone())
                } else {
                    None
    ...
    if let Some(ValueCell::Object(map)) = value_store.get_mut(container_id) {
        ...
        obj_rc
            .borrow_mut()
            .insert(key.clone(), load_value(value_id, value_store, heavy_store));
    }
    ...
    if key == "metadata" {
        ...
        let class_name_opt = obj_rc.borrow().get("__class_name").and_then(|v| {
            if let Value::String(s) = v {
                Some(s.clone())
            } else {
                None
        ...
        if is_meta && class_name_opt.is_some() {
            if let Value::Object(meta_rc) = &value {
                ...
                if let (Some(name), Some(Value::Object(classes_rc))) =
                    (class_name_opt, meta.get("classes"))
                {
                    classes_rc
                        .borrow_mut()
                        .insert(name, Value::Object(Rc::clone(&obj_rc)));
                }
            }
        }
    }
