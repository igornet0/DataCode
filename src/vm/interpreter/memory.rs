...
pub(crate) fn op_load_global(
    ...
    if frame.function.name == "<main>"
        ...
        && unsafe { (*vm_ptr).get_argv_old_indices() }
    ...
    if effective_index >= globals.len() {
        ...
        let mut id = if loading_argv {
            ...
            let id_from_slot = argv_slot.and_then(|slot_idx| {
                (slot_idx < globals.len())
                    .then(|| globals[slot_idx].resolve_to_value_id(value_store))
            });
            if in_main_chunk {
        ...
        if loading_argv && frame.function.name == "<main>" {
            ...
            let canonical_id = crate::vm::run_context::RunContext::get_script_argv_value_id()
                .or_else(|| {
                    crate::vm::run_context::RunContext::get_restored_script_argv_after_import()
                })
        ...
        }
        {
                ...
                && frame.function.name.contains("::new_")
            {
                ...
                .map(|c| matches!(c, ValueCell::Array(_)))
                .unwrap_or(false)
            {
                let ctor_name = frame
                    .function
                ...
                if ctor_name.map(|n| n.contains("::new_")).unwrap_or(false) {
                    ...
                    let candidate_indices: Vec<usize> = global_names
                        .iter()
            ...
            if crate::common::debug::is_debug_enabled() {
                if let Some(var_name) = global_names.get(&effective_index) {
                    if var_name.contains("Data")
                        || var_name.contains("range")
                        || var_name.contains("print")
                        || var_name.contains("::new_")
                    {
                        ...
                        debug_println!(
                            "[DEBUG LoadGlobal] Загружаем '{}' из globals[{}], значение: {}",
                            var_name,
                            index,
                            value_type_str
                        );
                    ...
                    if id_to_push == NULL_VALUE_ID
                        && (var_name.contains("Data")
                            || var_name.contains("range")
                            || var_name.contains("print"))
                    {
                        debug_println!(
                            "[DEBUG LoadGlobal] WARNING: '{}' в globals[{}] равен Null",
                            var_name,
                            index
                        );
                    }
...
pub(crate) fn op_store_global(
    ...
    if name_at_idx.map(|n| n.contains("::new_0")).unwrap_or(false) {
        ...
        if crate::common::debug::verbose_constructor_debug() {
            eprintln!(
                "[StoreGlobal] idx={} name={:?} value_type={}",
                index, name_at_idx, value_type
            );
        }
    }
    // Do not allow script to overwrite the argv slot (host-only, read-only).
    ...
    let from_constant = frames
        .last()
        .map(|f| f.constant_ids.contains(&value_id))
        .unwrap_or(false);
                        ...
                        if is_meta {
                            ...
                            if let Some(Value::Object(classes_rc)) = classes_rc_opt {
                                ...
                                loop {
                                    ...
                                    let ancestor_idx =
                                        global_index_by_name(global_names, &current_name);
                                    let (ancestor_has_meta, next_super) = if let Some(idx) =
                                        ancestor_idx
                                    {
                                        if idx < globals.len() {
