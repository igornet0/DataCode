...
pub fn update_chunk_indices_from_names(
    ...
    let mut name_index_pairs: Vec<_> = chunk
        .global_names
        .iter()
        .map(|(i, n)| (*i, n.clone()))
        .collect();
    ...
    for (old_idx, name) in &name_index_pairs {
        ...
        let real_idx: Option<usize> = if name == "argv" && argv_slot_index.is_some() {
            ...
            if matching_indices.is_empty() {
                ...
                if let (Some(globals), Some(store), Some(heap)) = (
                    globals_for_verify.as_deref_mut(),
                    store.as_deref_mut(),
                    heap,
                ) {
                    ...
                    let non_null: Vec<usize> = matching_indices
                        ...
                        .filter(|&&idx| {
                            idx >= BUILTIN_COUNT
                                ...
                                && !matches!(
                                    load_value(
                                        globals[idx].resolve_to_value_id(store),
                                        store,
                                        heap
                                    ),
                                    Value::Null
        ...
        if let Some(r) = real_idx {
            ...
            if *old_idx != r_final {
                ...
                if !would_overwrite || force_argv {
                    ...
                    debug_println!(
                        "[DEBUG update_chunk_indices] Маппинг '{}': {} -> {} (argv forced={})",
                        name,
                        old_idx,
                        r_final,
                        name == "argv" && argv_slot_index.is_some()
                    );
                } else {
    ...
    if crate::common::debug::verbose_constructor_debug() {
        for (old_idx, name) in &name_index_pairs {
            if let Some(&real_idx) = old_to_real.get(old_idx) {
                eprintln!(
                    "[update_chunk_indices] '{}' old_idx {} -> real_idx {}",
                    name, old_idx, real_idx
                );
            } else {
        ...
        if let Some(name) = chunk.global_names.get(&MODEL_CONFIG_CLASS_LOAD_INDEX) {
            ...
            let real_idx = if name.as_str() == "__constructing_class__" {
                ...
                class_slots
                    .into_iter()
                    .max()
                    .or_else(|| matching_indices.into_iter().max())
            };
    ...
    if !old_to_real.is_empty() {
        debug_println!(
            "[DEBUG update_chunk_indices] Обновлено {} инструкций по {} маппингам",
            updated_count,
            old_to_real.len()
        );
    }
