...
pub fn native_sha256(args: &[Value]) -> Value {
    ...
    hasher.update(&data);
    Value::ByteBuffer(ByteBuffer::from_vec(hasher.finalize().to_vec()))
}
...
pub fn native_hmac_sha256(args: &[Value]) -> Value {
    ...
    mac.update(&data);
    Value::ByteBuffer(ByteBuffer::from_vec(mac.finalize().into_bytes().to_vec()))
}
...
pub fn native_random_int(args: &[Value]) -> Value {
    ...
    let min = match &args[0] {
        Value::Number(x) if x.is_finite() && x.fract() == 0.0 => *x as i64,
        _ => {
    ...
    let v: i64 = rand::rngs::OsRng.gen_range(min..=max);
