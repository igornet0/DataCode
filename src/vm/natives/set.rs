...
/// `set()` → empty set; `set([a, b, ...])` → set from array elements (must be hashable).
...
pub fn native_set_update(args: &[Value]) -> Value {
    ...
    unsafe {
        let vm = &mut *vm_ptr;
        let coerced = match crate::vm::iterable::coerce_to_iterable_value(iterable) {
            Ok(v) => v,
            Err(e) => {
                crate::websocket::set_native_error(e.to_string());
                return Value::Set(Rc::clone(s));
            }
        };
        let Value::Iterable(rc) = coerced else {
            crate::websocket::set_native_error(
                "TypeError: set.update() expected an iterable".to_string(),
            );
            return Value::Set(Rc::clone(s));
        };
        loop {
            let next = {
                ...
                match crate::vm::iterable::iterable_next(&mut *inner, vm) {
                    Ok(n) => n,
                    Err(e) => {
                        crate::websocket::set_native_error(e.to_string());
                        return Value::Set(Rc::clone(s));
