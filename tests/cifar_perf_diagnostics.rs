//! CIFAR bin diagnostics: read_file_bin → chunk/slice → lists (see tests/diagnostics/cifar_bin_diagnostics.dc).
...
//! - `array.chunk(n)` on `read_file_bin` output (`ByteBuffer`): each window is a zero-copy byte slice (`materialize_chunk_at` in `src/vm/iterable.rs`); chunk over a heap `Array` still clones elements.
...
ch0[0]"#,
    ...
    match v {
        Value::Number(n) => assert!((0.0..=255.0).contains(&n), "byte value {}", n),
        other => panic!("expected Number for chunk[0] label byte, got {:?}", other),
    }
}

/// `read_file_bin` → slice (`ArrayView`) → `chunk(n)`: each row must be an owned array; first byte + tail length.
...
ch[0] + len(ch[1:])"#,
    ...
    let v = run_with_base_path(&src, base.as_path()).expect("run");
    match v {
        Value::Number(n) => assert!((n - 3.0).abs() < 1e-9, "expected 3.0 (1+2), got {}", n),
        other => panic!("expected Number, got {:?}", other),
    }
}
...
"#,
    ...
    match v {
        Value::Number(n) => assert!(
            (n - 3.0).abs() < 1e-9,
            "expected score 3 (int,int labels + 3072 tail), got {}",
            n
        ),
        other => panic!("expected Number score, got {:?}", other),
    }
}
...
"#,
    ...
    match v {
        Value::Number(n) => assert!(
            (n - 3.0).abs() < 1e-9,
            "expected score 3 inside fn, got {}",
            n
        ),
        other => panic!("expected Number score, got {:?}", other),
    }
