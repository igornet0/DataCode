#!/usr/bin/env python3
"""Generate tests/dcp_fixtures/with_sql.dcp (DCP v1.1 with __sql__ section)."""
...
SQL = "CREATE VIEW v_names AS SELECT name FROM t;"
...
def build_dcp(code: str, sql: str) -> bytes:
    strings = ["__code__", "__sql__"]
...
def main() -> None:
    dcp = build_dcp(CODE, SQL)
    out_path = Path(__file__).with_name("with_sql.dcp")
